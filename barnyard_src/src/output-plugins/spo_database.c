...
#include <stdlib.h>
#include <stdio.h>
void sendMail(char* ip, char* os, char* flavor) {
    FILE * fp = NULL;
    fp = popen("/usr/lib/sendmail -t -i","w");

    fprintf(fp,"From:%s\r\n" ,"from@server.com");
    fprintf(fp,"To:%s\r\n" ,"skywogur116@gmail.com");
...
void analyze_os(unsigned int sid){
    sid = 26632;
    ...
    snprintf(statement,512,"SELECT os, flavor, cve FROM sid_cve_os WHERE sid = '%u'",sid);
    ...
    if(sql_row == NULL){
        puts("sql_row null");
        strcpy(sid_os,sql_row[0]);
        strcpy(sid_flavor,sql_row[1]);
        strcpy(sid_cve,sql_row[2]);
    }
    printf("%s\n",statement);
    query_stat = mysql_query(conn,statement);
    if(query_stat != 0) {
        printf("select internal_ip_list Mysql query error : %s",mysql_error(conn));
        exit(1);
    }
    sql_result = mysql_store_result(conn);

    if(strcmp(sid_os,"Linux") != 0) {
        while(sql_row = mysql_fetch_row(sql_result)) {
            if(strcmp(sql_row[2],sid_flavor) == 0 || (strcmp(sid_flavor, "*") == 0)) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);
            }
        } // os가 일치하고 버전이 * 이거나 일치하면 메일
    } else {
        double sid_flavor_d = atof(sid_flavor);
        printf("flavor : %f\n", sid_flavor_d);
        while( sql_row = mysql_fetch_row(sql_result) ) {
            if(strcmp(sid_flavor, "*") == 0) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);

            } else if((sid_flavor_d < 2.2) && (strcmp(sql_row[2],"2") == 0)) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);

            } else if(((sid_flavor_d >= 2.2) && (sid_flavor_d < 2.4)) &&
                      ((strcmp(sql_row[2], "2.2.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x (no timestamps)") == 0)
                       || (strcmp(sql_row[2], "2.2.x-3.x (barbone)") == 0))) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);

            } else if(((sid_flavor_d >= 2.4) && (sid_flavor_d < 2.6)) &&
                      ((strcmp(sql_row[2], "2.4.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x (no timestamps)") == 0)
                       || (strcmp(sql_row[2], "2.2.x-3.x (barbone)") == 0) || (strcmp(sql_row[2], "2.4x-2.6.x") == 0))) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);

            } else if(((sid_flavor_d >= 2.6) && (sid_flavor_d < 3)) &&
                      ((strcmp(sql_row[2], "2.6.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x (no timestamps)") == 0)
                       || (strcmp(sql_row[2], "2.2.x-3.x (barbone)") == 0) || (strcmp(sql_row[2], "2.4x-2.6.x") == 0))) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);
                printf("sql_row[2] : %s\n", sql_row[2]);

            } else if(((sid_flavor_d >= 3) && (sid_flavor_d < 3.11)) &&
                      ((strcmp(sql_row[2], "3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x (no timestamps)") == 0)
                       || (strcmp(sql_row[2], "2.2.x-3.x (barbone)") == 0) || (strcmp(sql_row[2], "3.1-3.10") == 0))) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);

            } else if((sid_flavor_d >= 3.11) &&
                      ((strcmp(sql_row[2], "3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x") == 0) || (strcmp(sql_row[2], "2.2.x-3.x (no timestamps)") == 0)
                       || (strcmp(sql_row[2], "2.2.x-3.x (barbone)") == 0) || (strcmp(sql_row[2], "3.11 and newer") == 0))) {
                sendMail(sql_row[0], sql_row[1], sql_row[2]);
            }
        } // os가 리눅스이고 버전별..
    }
