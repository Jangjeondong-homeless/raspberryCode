...
struct tcp_sig* fingerprint_tcp(u8 to_srv, struct packet_data* pk,
 ...
 char update_statement[512];
  ...
  if ((m = sig->matched)) {
    ...
    u8* db_ip = addr_to_str(f->client->addr, f->client->ip_ver);
    if(to_srv){
	    snprintf(statement,512,"SELECT * FROM internal_ip_list WHERE ip = '%s'",db_ip);
	    ...
	    if(sql_row == NULL) {
		    snprintf(insert_statement,512,"INSERT INTO internal_ip_list (ip,os,flavor)  VALUES ('%s','%s','%s')",db_ip,fp_os_names[m->name_id],m->flavor);
		    int db_insert_result = mysql_query(conn,insert_statement);
		    if(db_insert_result != 0) {
			    printf("insert Mysql query error : %s", mysql_error(conn));
			    exit(1);
		    }
	    }
	    else {
		    snprintf(update_statement,512,"UPDATE internal_ip_list SET os = '%s', flavor = '%s', update_time = CURRENT_TIMESTAMP WHERE ip = '%s'",fp_os_names[m->name_id],m->flavor,db_ip);
		    int db_update_result = mysql_query(conn,update_statement);
		    if(db_update_result != 0) {
			    printf("update Mysql query error : %s", mysql_error(conn));
			    exit(1);
		    }
	    }
    }
